...
function angle(vehicle)
	...
	if GetEntitySpeed(vehicle)* 3.6 < 40 or GetVehicleCurrentGear(vehicle) == 0 then
		return 0, modV --speed over 25 km/h
	end
	...
	while true do
		...
		if getCurrentSkin() == skinData.skinName then
			...
			if DoesEntityExist(vehicle) and not IsEntityDead(vehicle) then
				...
				for i,name in ipairs(Config.SpecialCars) do
					if GetDisplayNameFromVehicleModel(GetEntityModel(vehicle)) == name then
						labelType = "86"
						cst.rpmScale = 242
