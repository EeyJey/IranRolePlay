...
function AlertThePolice()
	...
	if event_is_running then
    	...
    	TriggerServerEvent('esx_addons_gcphone:startCall', 'police', ":Tavajoh: Havasetoon bashe! ye mashin ba pelake [ " .. vehicle_plate .. " ] oomade!", PlayerCoords, {
    	...
    	for i = 1, #Config.AlertExtraSocieties do
    		...
    		if PlayerData.job.name ~= Config.AlertExtraSocieties[i] then
    			...
    			TriggerServerEvent('esx_addons_gcphone:startCall', Config.AlertExtraSocieties[i], ":Tavajoh: Havasetoon bashe! ye mashin ba pelake [ " .. vehicle_plate .. " ]  oomade!", PlayerCoords, {
				PlayerCoords = { x = PedPosition.x, y = PedPosition.y, z = PedPosition.z },
				})
