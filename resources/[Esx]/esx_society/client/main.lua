...
function getSocietyMoneyClient()
	if ESX.PlayerData.job.grade_name == 'boss' then
		ESX.TriggerServerCallback('esx_society:getSocietyMoney', function(money)
			societyBankMoney = money
		end, ESX.PlayerData.job.name)
	end
	return societyBankMoney
end
...
function OpenBossMenu(society, close, options)
	...
	if options.bank then
		local formattedMoney = _U('locale_currency', ESX.Math.GroupDigits(societyBankMoney))
		table.insert(elements, {label = ('%s: <span style="color:green;">%s</span>'):format(_U('check_bank'), formattedMoney), value= 'check_society_money'})
	end
